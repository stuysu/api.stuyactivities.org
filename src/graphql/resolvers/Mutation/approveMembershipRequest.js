...
export default async (parent, args, context) => {
	...
	const {session, models: {memberships, membershipRequests}} = context
	...
	if (!isClubAdmin(session.userId, orgId, memberships)) {
		throw new ForbiddenError("You do not have the right to remove members from this club!")
	}

	const membershipRequest = await membershipRequests.findOne({
		where: {
			organizationId: orgId, userId, userApproval: true
		}
	})
